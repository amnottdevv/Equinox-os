// ============================================================
//  equinox_anchors.c — TLS trust anchors for Equinox OS (mget)
// ------------------------------------------------------------
//  GENERATED FILE — produced by the upstream `brssl ta` tool:
//    brssl ta -q selected_roots.pem > anchors.c
//  Source: Debian ca-certificates (Mozilla root store), reduced
//  to the 9 roots BearSSL 0.6 can actually use on this target
//  (RSA of any size + ECDSA P-256 only; P-384/Ed25519 roots are
//  dropped because the engine cannot verify their keys).
//  Regenerate: see anchors/README.txt
//
//  Consumed by kernel/net/tls_client.c (strict verify mode).
//  Compiled as plain C by the makefile BearSSL rule (it uses
...
static const br_x509_trust_anchor TAs[9] = {
	{
...
// ---- Equinox OS export wrappers (non-static, C linkage) ----
// tls_client.c (C++) reads these through tls_client.h.
const br_x509_trust_anchor* const eq_tls_anchors = TAs;
const unsigned eq_tls_anchors_num = (unsigned)TAs_NUM;
